...
export function AdminDashboard() {
  ...
  const showcaseProducts = data.catalog.products.filter((product) => product.inShowcase).sort((left, right) => left.showcaseOrder - right.showcaseOrder);
  const showcaseIds = showcaseProducts.map((product) => product.id);
      ...
      <section className="admin-content">
        ...
        {activePanel === "dashboard" && <div className="admin-panel-view">
          ...
          <section className="dashboard-shortcuts"><div className="section-heading"><div><p className="eyebrow">Acesso rápido</p><h2>O que deseja fazer?</h2></div></div><div className="shortcut-grid">
            ...
            <button onClick={() => openPanel("showcase")}><span><GalleryHorizontal size={22}/></span><b>Editar showcase</b><small>{showcaseProducts.length} de 5 produtos</small></button>
        ...
        {activePanel === "products" && <section className="admin-panel-view">
          <div className="admin-toolbar"><label className="admin-search"><Search size={18}/><input type="search" value={productSearch} onChange={(event) => setProductSearch(event.target.value)} placeholder="Buscar produto"/></label><div className="admin-segments">{([['all','Todos'],['active','Ativos'],['paused','Pausados'],['featured','Destaques']] as Array<[ProductFilter,string]>).map(([id,label]) => <button key={id} className={productFilter === id ? "is-active" : ""} onClick={() => setProductFilter(id)}>{label}</button>)}</div><button className="button button-dark" onClick={openNewProduct}><Plus size={16}/>Novo produto</button></div>
          ...
          <div className="admin-product-grid">{filteredProducts.map((product) => <article key={product.id} className={!product.isAvailable ? "is-paused" : ""}>
            <div className="admin-product-photo"><Image src={product.imageUrl} alt="" fill sizes="(max-width: 619px) 100vw, (max-width: 1179px) 50vw, 25vw"/>{product.featured && <span><Star size={12} fill="currentColor"/>Destaque</span>}</div>
        ...
        {activePanel === "showcase" && <section className="admin-panel-view showcase-manager">
          <div className="showcase-intro"><div><GalleryHorizontal size={24}/><span><b>Banner da página inicial</b><small>Selecione até 5 produtos. A foto principal, nome, descrição e preço formam cada slide.</small></span></div><strong>{showcaseProducts.length}/5</strong></div>
          <div className="showcase-layout"><section><div className="section-heading compact"><div><p className="eyebrow">Ordem do banner</p><h2>Slides ativos</h2></div></div>{showcaseProducts.length ? <div className="showcase-selected-list">{showcaseProducts.map((product, index) => <article key={product.id}>
            <span className="showcase-position">{index + 1}</span><div className="showcase-thumb"><Image src={product.imageUrl} alt="" width={56} height={44}/></div><div><b>{product.name}</b><small>{formatCurrency(product.priceCents)}</small></div><div className="showcase-row-actions"><button disabled={index === 0 || busyId === "showcase"} onClick={() => { const ids = [...showcaseIds]; [ids[index - 1], ids[index]] = [ids[index], ids[index - 1]]; void saveShowcase(ids); }} aria-label="Mover para cima" title="Mover para cima"><ArrowUp size={15}/></button><button disabled={index === showcaseProducts.length - 1 || busyId === "showcase"} onClick={() => { const ids = [...showcaseIds]; [ids[index], ids[index + 1]] = [ids[index + 1], ids[index]]; void saveShowcase(ids); }} aria-label="Mover para baixo" title="Mover para baixo"><ArrowDown size={15}/></button><button className="danger" disabled={busyId === "showcase"} onClick={() => void saveShowcase(showcaseIds.filter((id) => id !== product.id))} aria-label="Remover do showcase" title="Remover"><Trash2 size={15}/></button></div>
