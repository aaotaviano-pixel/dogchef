...
import { ArrowLeft, Check, ChevronDown, Clock3, CreditCard, MapPin, Minus, Plus, ShoppingBag, X } from "lucide-react";
  ...
  return (
    <main className="store-shell dogchef-store">
      <header className="store-header">
        <div className="brand-lockup"><span className="brand-mark">D</span><span><strong>DogChef</strong><small>feito na hora, do seu jeito</small></span></div>
        <button className="icon-button" aria-label="Abrir carrinho" onClick={() => setCartOpen(true)}><ShoppingBag size={20}/>{totalItems > 0 && <b className="cart-count">{totalItems}</b>}</button>
      </header>

      <section className="hero dogchef-hero menu-reveal menu-reveal--title">
        <p className="eyebrow"><span className="dot"/> {catalog.acceptingOrders ? "Pedidos abertos" : "Loja pausada"}</p>
        <h1>Prensado de verdade,<br/><em>sabor de respeito.</em></h1>
        <p>Hot dogs, gratinados, porções e bebidas preparados na hora para você.</p>
          ...
          return <section key={category.id} className="dogchef-menu-section" aria-labelledby={`category-${category.id}`}>
            ...
            <div className="product-grid">
              {categoryProducts.map((product) => <article key={product.id} className={`product-card menu-reveal ${!product.isAvailable ? "unavailable" : ""}`}>
                <div className="product-top"><span className="food-emoji">{product.emoji}</span>{product.featured && <span className="pill">{product.highlight ?? "destaque"}</span>}</div>
      ...
      {selectedProduct && <div className="overlay" role="dialog" aria-modal="true" aria-label={`Personalizar ${selectedProduct.name}`}>
        <div className="bottom-sheet configurator">
          ...
          <span className="food-emoji large">{selectedProduct.emoji}</span><p className="eyebrow">Personalize</p><h2>{selectedProduct.name}</h2><p className="muted">{selectedProduct.description}</p>
      ...
      {checkout && <div className="overlay" role="dialog" aria-modal="true" aria-label="Finalizar pedido"><form className="bottom-sheet checkout-sheet" onSubmit={submitOrder}><button type="button" className="sheet-close" onClick={() => setCheckout(false)} aria-label="Fechar"><X size={21}/></button><p className="eyebrow">Finalizar pedido</p><h2>Só faltam seus dados</h2><div className="checkout-summary"><span>{totalItems} itens</span><strong>{formatCurrency(subtotal)}</strong></div><div className="form-grid"><label className="field"><span>Seu nome</span><input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} autoComplete="name"/></label><label className="field"><span>WhatsApp</span><input required value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} inputMode="tel" placeholder="(00) 00000-0000" autoComplete="tel"/></label><label className="field full-width"><span>E-mail <small>obrigatório somente para Pix</small></span><input value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} type="email" autoComplete="email" placeholder="voce@email.com"/></label></div><fieldset className="choice-field"><legend>Como você recebe?</legend><div className="segmented"><button type="button" className={form.deliveryType === "delivery" ? "selected" : ""} onClick={() => setForm({ ...form, deliveryType: "delivery" })}>Entrega</button><button type="button" className={form.deliveryType === "pickup" ? "selected" : ""} onClick={() => setForm({ ...form, deliveryType: "pickup" })}>Retirar no balcão</button></div></fieldset>{form.deliveryType === "delivery" && <div className="address-fields"><label className="field street"><span>Rua / Avenida</span><input required value={form.street} onChange={(event) => setForm({ ...form, street: event.target.value })}/></label><label className="field number"><span>Número</span><input required value={form.number} onChange={(event) => setForm({ ...form, number: event.target.value })}/></label><label className="field"><span>Bairro</span><input required value={form.neighborhood} onChange={(event) => setForm({ ...form, neighborhood: event.target.value })} placeholder="Consulte as zonas atendidas"/></label><label className="field"><span>Complemento</span><input value={form.complement} onChange={(event) => setForm({ ...form, complement: event.target.value })}/></label><label className="field full-width"><span>Ponto de referência</span><input value={form.reference} onChange={(event) => setForm({ ...form, reference: event.target.value })}/></label></div>}<fieldset className="choice-field"><legend>Forma de pagamento</legend><label className={`payment-option ${form.paymentMethod === "cash" ? "selected" : ""}`}><input type="radio" checked={form.paymentMethod === "cash"} onChange={() => setForm({ ...form, paymentMethod: "cash" })}/><span><b>Dinheiro</b><small>Você paga na entrega/retirada</small></span><span className="payment-symbol">$</span></label><label className={`payment-option ${form.paymentMethod === "card" ? "selected" : ""}`}><input type="radio" checked={form.paymentMethod === "card"} onChange={() => setForm({ ...form, paymentMethod: "card" })}/><span><b>Cartão na entrega</b><small>Informe a maquininha ao receber</small></span><CreditCard size={21}/></label><label className={`payment-option disabled ${form.paymentMethod === "pix" ? "selected" : ""}`}><input type="radio" disabled={!catalog.pixConfigured} checked={form.paymentMethod === "pix"} onChange={() => setForm({ ...form, paymentMethod: "pix" })}/><span><b>Pix</b><small>{catalog.pixConfigured ? "QR Code após confirmar" : "Aguardando configuração"}</small></span><span className="pix-mark">PIX</span></label></fieldset>{formError && <p className="form-error">{formError}</p>}<button className="button button-primary full" disabled={isSubmitting}>{isSubmitting ? "Enviando pedido…" : <>Enviar pedido <span>{formatCurrency(subtotal)}</span></>}</button><p className="privacy-note">Ao enviar, você aceita que a DogChef use seus dados apenas para preparar e entregar este pedido.</p></form></div>}
    </main>
  );
}
