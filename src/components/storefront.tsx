  ...
  return (
    <main className="store-shell dogchef-store storefront-reference-redesign">
      ...
      <footer className="store-footer">
        <div className="store-footer-brand menu-reveal"><div className="brand-lockup"><span className="brand-mark"><ChefHat size={20}/></span><span><strong>Dog do Chef</strong><small>hot dog prensado</small></span></div><p>Cardápio online com adicionais, retirada, entrega e acompanhamento do pedido.</p></div>
        <div className="store-footer-service menu-reveal"><span>Atendimento</span><strong>{catalog.hoursLabel}</strong><small>Entrega padrão {formatCurrency(catalog.defaultDeliveryFeeCents)}</small></div>
        <div className="store-footer-contact menu-reveal"><span>Canais da loja</span><a href="https://www.instagram.com/dogdochef_prensado/" target="_blank" rel="noreferrer"><InstagramLogo size={17}/>@dogdochef_prensado</a><a className={catalog.whatsappConfigured ? "store-footer-whatsapp" : "store-footer-whatsapp is-pending"} href={catalog.whatsappUrl || WHATSAPP_PENDING_URL} target="_blank" rel="noreferrer" aria-label="Falar com a loja pelo WhatsApp" onClick={(event) => { if (!catalog.whatsappConfigured) event.preventDefault(); }}><WhatsAppLogo size={17}/>{catalog.whatsappConfigured ? "WhatsApp da loja" : "WhatsApp em configuração"}</a></div>
        <nav className="store-footer-links menu-reveal" aria-label="Informações da loja"><Link href="/politica-de-privacidade">Privacidade</Link><Link href="/termos-de-uso">Termos de uso</Link></nav>
      </footer>
