...
import { ArrowRight, LogIn, LockKeyhole, UserPlus } from "lucide-react";
...
export function CustomerAccess({ onAuthenticated, googleReturnTo }: { onAuthenticated: (customer: CustomerAccount) => void; googleReturnTo?: string }) {
  ...
  return <section className="customer-access">
    <div className="customer-access-heading"><span>{mode === "login" ? <LockKeyhole size={19}/> : <UserPlus size={19}/>}</span><div><b>{mode === "login" ? "Entre na sua conta" : "Crie sua conta"}</b><small>Seus pedidos ficam salvos para consultar quando quiser.</small></div></div>
    <div className="customer-access-tabs" role="tablist" aria-label="Acesso à conta">
